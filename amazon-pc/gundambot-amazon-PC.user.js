...
(function () {
            ...
            try {
                    ...
                    : products.map((p, idx) => {
                        ...
                        return '<div data-row-asin="' + escHtml(p.asin) + '" draggable="true" class="lb-am-prod-row" style="display:flex;align-items:center;gap:10px;padding:8px 10px;margin-bottom:5px;background:rgba(8,18,26,0.75);border-left:4px solid ' + accentColor + ';border-radius:4px;">' +
                            ...
                            '<div style="flex:0 0 auto;display:flex;gap:5px;align-items:center;">' +
                                ...
                                // ★PC-1.3.0: 1 商品だけを PC⇔スマホで受け渡すコピー (HIRO 要望 2026-09-01)
                                '<button data-asin="' + escHtml(p.asin) + '" class="lb-am-prod-copy1" title="この商品だけコピー(スマホへ渡す)" style="padding:6px 9px;background:rgba(46,196,127,0.2);border:1px solid rgba(46,196,127,0.6);color:#6ee7a8;border-radius:4px;font-size:14px;">📋</button>' +
                ...
                ov.innerHTML =
                    ...
                    '<div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;">' +
                        ...
                        // ★PC-1.3.0: 1 商品だけの貼り付け登録 (各行の 📋 で作った文字列を貼る)
                ...
                // ★PC-1.3.0: 📋 1 商品を貼り付け登録 (スマホの 📋 コピーで作った文字列を貼る)
                //   中身は「ヘッダ + 1 行のミニ CSV」なので既存 importProductsFromCsv() をそのまま利用。
                //   ヘッダ無し(データ行のみ)で貼られた場合も救済する。
                ...
                // ★PC-1.3.0: 📋 1 商品だけコピー (スマホへ渡す用)
                //   形式は既存 CSV と完全互換(ヘッダ + 1 行)。buynow_url ごと運べるのが要点。
                Array.prototype.forEach.call(ov.querySelectorAll('.lb-am-prod-copy1'), (btn) => {
                    btn.addEventListener('click', async () => {
                        ...
                        if (ok) {
                            ...
                            try { prompt('コピーしてください(Ctrl+C):', text); } catch (e) {}
                        }
                    });
                });
    ...
    const main = async () => {
        ...
        // ★PC-1.1.5: スポンサー広告の画像を誤取得していた分を 1 回だけ全クリア。
        //   fetchProductMeta を「商品メイン #landingImage 優先」に直したので、次回 商品ページ/🏠 訪問で
        //   正しい商品画像を取り直す。画像は飾り用データのみ・再取得は軽量なので全消去で安全。1 回限り。
        try {
            if (!localStorage.getItem('LB_AM_MIG_IMG_PC115')) {
                const imgKeys2 = [];
                for (let i = 0; i < localStorage.length; i++) {
                    const k = localStorage.key(i);
                ...
                localStorage.setItem('LB_AM_MIG_IMG_PC115', String(Date.now()));
