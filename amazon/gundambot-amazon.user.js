...
(function () {
    ...
    // ★v0.4.5.0 確定 click 直後の「凍結」(HIRO 指示 2026-09-04)
    ...
    const extractProductImage = () => {
        try {
            const getters = [
                // ★v0.3.9.3: スポンサー広告の誤取得を根絶。実DOM(B07P6P55FM)で検証:
    ...
    const fetchProductMeta = async (asin) => {
        ...
        try {
            ...
            // ── 画像 (★v0.3.9.2: 実HTML(B07P6P55FM)で検証して確定。#landingImage の<img>タグ全体から抽出) ──
    ...
    const startPurchaseTransAm = async () => {
        ...
        // ★v0.4.4.0 案A: 保存済み直販 URL があれば ?m= 切替をスキップして直撃する
        //
    ...
    const performOrderConfirm = async (rootEl, contextLabel) => {
        ...
        // ★v0.3.9.4: TRANS-AM(b方式)は保存済み offerListing.1 = Amazon直販Buy Boxの「今すぐ買う」由来。
        //   turbo の SPC 確定画面は「販売元 Amazon.co.jp」「Amazon発送」表示が出ない構造のことがあり、
        //   verifyCheckoutSafety が直販を確認できず false negative になる(PC版 B09Q6GH9HN で実証)。
        //   TRANS-AM中は販売元/発送テキスト欠落のみ救済。マケプレ積極検出/複数商品/価格超過は従来どおり拒否。
    ...
    const handleStockOutBuyNow = async () => {
        ...
        // ★v0.4.5.0 確定 click 直後は navigate しない (HIRO 指示: 確実に処理させる)
    ...
    const main = async () => {
        ...
        // ★v0.3.9.3: ライブDOM経路(extractProductImage)が #landingImage より上の関連/スポンサー
        //   画像を誤取得していた分を 1 回だけ全クリア。商品ページ訪問のたびに上書き保存されるため
        //   .391 のクリア後も再混入していた。抽出を商品メイン画像コンテナ限定に直したので、
        //   次回 商品ページ/🏠 で正画像を取り直す。1 回限り。
        try {
            if (!localStorage.getItem('LB_AM_MIG_IMG_V393')) {
                const imgKeys3 = [];
                for (let i = 0; i < localStorage.length; i++) {
                    const k = localStorage.key(i);
                ...
                localStorage.setItem('LB_AM_MIG_IMG_V393', String(Date.now()));
                if (imgKeys3.length > 0) {
                    try { logAm('info', 'migration',
                        'v0.3.9.3: 広告誤取得(ライブDOM)分を全クリア → 次回 商品ページ/🏠 で正画像を取り直し',
